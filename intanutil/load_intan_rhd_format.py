...
def read_data(filename):
    ...
    if data_present:
        ...
        data['amplifier_data'] = np.zeros([header['num_amplifier_channels'], num_amplifier_samples], dtype=np.float32)
        data['aux_input_data'] = np.zeros([header['num_aux_input_channels'], num_aux_input_samples], dtype=np.float32)
        data['supply_voltage_data'] = np.zeros([header['num_supply_voltage_channels'], num_supply_voltage_samples], dtype=np.float32)
        data['temp_sensor_data'] = np.zeros([header['num_temp_sensor_channels'], num_supply_voltage_samples], dtype=np.float32)
        data['board_adc_data'] = np.zeros([header['num_board_adc_channels'], num_board_adc_samples], dtype=np.float32)
    ...
    if (data_present):
        ...
        # Scale voltage levels appropriately. This is done in read_one_data_block for efficiency
        
        # data['amplifier_data'] = np.multiply(0.195, (data['amplifier_data'].astype(np.int32, copy=False) - 32768))      # units = microvolts
